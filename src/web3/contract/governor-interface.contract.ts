...
import {
    BaseMultiChainContract, BlockchainDefinition, DefaultEVMNativeTokenDecimals, MethodRunnable,
...
export class GovernorInterfaceContract extends BaseMultiChainContract {
    ...
    public execute(
        ...
        return this.buildMethodRunnableMulti(
            governor,
            async (contract, _) => contract.methods.executeMethodCalls(targets, ValuesBN.map(x => x.toString()), calldatas, descriptionHash),
            async () => {
            },
            async () => total);
    }
    ...
    public propose(governor: string, targets: string[], values: BigNumber[], calldatas: string[], description: string) {
        ...
        return this.buildMethodRunnableMulti(
            governor,
            async (contract, _) => contract.methods.makeProposal(targets, ValuesBN.map(x => x.toString()), calldatas, description)
        );
    }
    ...
    public voteForProposal(governor: string, proposalId: string): MethodRunnable {
        return this.buildMethodRunnableMulti(
            governor,
            async (contract, _) => contract.methods.voteForProposal(proposalId)
        );
    }
    ...
    public async buildProposalId(
        ...
        const valuesStrings = values.map(x => new BigNumber(x).multipliedBy(DefaultEVMNativeTokenDecimals).toString());
