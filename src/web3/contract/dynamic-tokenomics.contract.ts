...
export class DymanicTokenomicsContractService extends BaseTokenAwareContract<DynamicTokenomicsAbiFunctional> {
  ...
  public async getPayees(
    ...
    return this.getViewMulti(config, address, async (contract) => contract.methods.getPayees(n), batch, callback);
  }
  ...
  public addPayee(address: string, n: number, account: string, shares: number, nativeShares: number) {
    return this.buildMethodRunnableMulti(address, async (contract, connectedAddress) =>
      contract.methods.addPayee(n, account, shares, nativeShares),
    );
  }

  public changePayeeShare(address: string, n: number, account: string, shares: number, nativeShares: number) {
    return this.buildMethodRunnableMulti(address, async (contract, connectedAddress) =>
      contract.methods.changePayeeShare(n, account, shares, nativeShares),
    );
  }

  public removePayee(address: string, n: number, account: string) {
    return this.buildMethodRunnableMulti(address, async (contract, connectedAddress) =>
      contract.methods.removePayee(account, n),
    );
  }
